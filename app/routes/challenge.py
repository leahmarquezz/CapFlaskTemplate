...
def challenge(challengeID):
    ...
    return render_template('challenge.html', challenge=thisChallenge)
...
def challengeNew():
    try:
        thisChallenge = Challenge.objects.get(id = current_user.id)
        ...
        return redirect(url_for('challengeEdit', challengeID = thisChallenge.id))
